...
window.log = (function () {
    ...
    const log_init = (params: any) => {
        ...
        let tempParams: any = parameter_correction(params);

        //If there is an ip, use the external ip
        if(!!tempParams.ip){
            storage.save('terminal_info', { ip: tempParams.ip, city: tempParams.city });
        }

        //Clean the data collected before startup
        storage.clear_all_log();
    ...
    const log_set_options = (params: any) => {
        ...
        //If there is an ip, use the external ip
        if(!!params.ip){
            storage.save('terminal_info', { ip: params.ip, city: params.city });
        }
