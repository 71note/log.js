...
const get_ip_api = (number: number) => {
    //Stop acquiring IP if it has already been acquired
    if (!!storage.terminal_info.ip || number > 3) return void 0;
    fetch(IGNORE_LIST[1]).then(res => res.json()).then(data => {

        if (data.status === 'fail') {
            return get_ip_souhu(number + 1);
        }

        let obj: object = {
            ip: data.query,
            city: data.country + '-' + data.regionName + '-' + data.city
        };
        storage.save("terminal_info", obj);
    }).catch(() => get_ip_souhu(number + 1));
}

const get_ip_chat_gpt = (number: number) => {
